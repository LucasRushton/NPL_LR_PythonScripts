...
#avoid_freq = [0.02, 0.05,0.25,1,2,5,10,20,30,75,100]
#avoid_freq = [0.02, 0.05,0.25,1,2,5,10,20,30,50,100]
#avoid_freq = [0.02, 0.05,0.25,1,2,5, 10, 20, 30]
avoid_freq = [0.05, 0.25, 2, 5, 10, 20, 30, 75, 100]
#avoid_freq = [0.02, 0.05, 0.25, 0.25, 1, 5, 30, 50, 100]

#theta = -80 * np.pi / 180#245 #260 * np.pi/180 #205 * np.pi/180
#theta_2 = 0#260 * np.pi/180 #205 * np.pi/180
#theta_x = -100 * np.pi / 180 #10 * np.pi / 180#245 #260 * np.pi/180 #205 * np.pi/180
...
by_offset = 0
bx_multiplier = 1#0.4
comsol_mur_of_interest = 100
comsol_sigma_of_interest = 0.001# 0.25E6 + 2 * 0.2E6
...
path1 = 'P:/Coldatom/LucasRushton/COMSOL/MagneticsPaper/230829_SharpEdges_Alu35MS'
textfile_name = '230829_SharpEdges_Alu35MS'
param_sweep_sheet_permeability = [1]
param_sweep_sheet_conductivity = [35E6]
comsol_step = 5.01
theta_rotated = 0 * np.pi / 180 #10 * np.pi / 180#245 #260 * np.pi/180 #205 * np.pi/180
param_sweep_x_pos_cell = [0]
param_sweep_sheet_height = [0.007]
...
path1 = 'P:/Coldatom/LucasRushton/COMSOL/MagneticsPaper/230829_SharpEdges_Alu20MS_LF1mm'
textfile_name = '230829_SharpEdges_Alu20MS_LF1mm'
param_sweep_sheet_permeability = [1]
param_sweep_sheet_conductivity = [20E6]
comsol_step = 5.01
theta_rotated = 0 * np.pi / 180 #10 * np.pi / 180#245 #260 * np.pi/180 #205 * np.pi/180
param_sweep_x_pos_cell = [0]
param_sweep_sheet_height = [0.001]
bx_offset = 0#5E-6

path1 = 'P:/Coldatom/LucasRushton/COMSOL/MagneticsPaper/230829_SharpEdges_Alu20MS_LF3mm'
textfile_name = '230829_SharpEdges_Alu20MS_LF3mm'
param_sweep_sheet_permeability = [1]
param_sweep_sheet_conductivity = [20E6]
comsol_step = 5.01
theta_rotated = 0 * np.pi / 180 #10 * np.pi / 180#245 #260 * np.pi/180 #205 * np.pi/180
...
param_sweep_x_pos_cell = [0]
param_sweep_sheet_height = [0.001]
bx_offset = 0.2E-6#5E-6

path1 = 'P:/Coldatom/LucasRushton/COMSOL/MagneticsPaper/230901_SharpEdges_Alu20MS_LF1mm_Step1'
textfile_name = '230901_SharpEdges_Alu20MS_LF1mm_Step1'
param_sweep_sheet_permeability = [1]
param_sweep_sheet_conductivity = [20E6]
comsol_step = 1
theta_rotated = 0 * np.pi / 180 #10 * np.pi / 180#245 #260 * np.pi/180 #205 * np.pi/180
param_sweep_x_pos_cell = [0]
param_sweep_sheet_height = [0.001]
bx_offset = 0#0.2E-6#5E-6

path1 = 'P:/Coldatom/LucasRushton/COMSOL/MagneticsPaper/230907_FR24mm_VaryingRecessDepth'
textfile_name = '230907_FR24mm_VaryingRecessDepth'
param_sweep_sheet_permeability = [100]
param_sweep_sheet_conductivity = [0.001]
comsol_step = 5
theta_rotated = 0 * np.pi / 180 #10 * np.pi / 180#245 #260 * np.pi/180 #205 * np.pi/180
param_sweep_x_pos_cell = [0]
param_sweep_sheet_height = [2.4, 0.5]
bx_offset = 0#0.2E-6#5E-6
            ...
            for i in range(len(param_sweep_freq)):
                ...
                if param_sweep_freq[i] in avoid_freq:
                    ...
                    if param_sweep_sheet_conductivity[i3]==comsol_sigma_of_interest and param_sweep_sheet_permeability[i2]==comsol_mur_of_interest:                
                        ...
                        ax5.plot(plate_length_conc, np.sqrt((np.real(bx_conc))**2+np.imag(bx_conc)**2), label='%skHz, $\mu_{r}=%s$, $\sigma=%s$MS/m' % (param_sweep_freq[i], param_sweep_sheet_permeability[i2], param_sweep_sheet_conductivity[i3]/10**6))
                        ax6.plot(plate_length_conc, 180 / np.pi * np.arctan2(np.imag(bx_conc), np.real(bx_conc)), label='%skHz, $\mu_{r}=%s$, $\sigma=%s$MS/m' % (param_sweep_freq[i], param_sweep_sheet_permeability[i2], param_sweep_sheet_conductivity[i3]/10**6))
                        ax7.plot(plate_length_conc, np.sqrt(np.real(by_conc)**2+np.imag(by_conc)**2), label='%skHz, $\mu_{r}=%s$, $\sigma=%s$MS/m' % (param_sweep_freq[i], param_sweep_sheet_permeability[i2], param_sweep_sheet_conductivity[i3]/10**6))
                        ax8.plot(plate_length_conc, 180/np.pi*np.arctan2(np.imag(by_conc), np.real(by_conc)), label='%skHz, $\mu_{r}=%s$, $\sigma=%s$MS/m' % (param_sweep_freq[i], param_sweep_sheet_permeability[i2], param_sweep_sheet_conductivity[i3]/10**6))
                        ...
                        ax5.set_title('$\mu_{r}=%s$, $\sigma=%s$MS/m, $B_{x}$ offset=%s' % (param_sweep_sheet_permeability[i2], param_sweep_sheet_conductivity[i3]/10**6, bx_offset))
                ...
                if param_sweep_freq[i] in avoid_freq:
                    ...
                    if param_sweep_sheet_conductivity[i3]==comsol_sigma_of_interest and param_sweep_sheet_permeability[i2]==comsol_mur_of_interest:                
                        ...
                        ax16.plot(plate_length_conc, R, label='COM, %skHz, LF=%sm' % (param_sweep_freq[i], param_sweep_sheet_height[i4]))
                        ax17.plot(plate_length_conc, 180/np.pi * Phase)
                        ...
                        max_comsol = max(R)
                        max_comsol_bx = max(np.abs(bx))
                        ax16.set_title('$\mu_{r}=%s$, $\sigma=%s$MS/m, $B_{x}$ offset=%s' % (param_sweep_sheet_permeability[i2], param_sweep_sheet_conductivity[i3]/10**6, bx_offset))
...
ax5.set_ylabel('|$B_{x}$| (T)')
ax6.set_ylabel('$B_{x}$ phase ($\degree$)')
ax7.set_ylabel('|$B_{y}$| (T)')
ax8.set_ylabel('$B_{y}$ phase ($\degree$)')
...
path_directory = 'P:/Coldatom/RafalGartman/230831'

#path_file = 'SinglePhotonPrimary_24mmRecessSteel_NoStSheet_40mVSubZero_B0Pump_VC93_LF_1mm_LS'
path_file = 'SingPh_B0Vert_24mmAlPlate_200mVSubZeroNoSwitch_100turn2mmFR_FieldComp_VC85_PIDnull_WritingTowardsDoor_LS'
...
for i in range(len(freq)):
    if freq[i] in avoid_freq:
        ...
        ax5.plot(np.arange(-150, 150, 300/len(x))-4, f1_all[i][:] * max_comsol_bx/max_exp, label='Exp, %s kHz' % freq[i])
        ax6.plot(np.arange(-150, 150, 300/len(x))-4, f2_all[i][:] - f2_all[i][0] + phase_offset_exp * 180/np.pi)
