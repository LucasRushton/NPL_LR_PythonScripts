...
# File path
path_directory = 'P:/Coldatom/RafalGartman/230907'
...
path_file = 'SingPh_B0Pump_24mmStWritingAwayFromComp_10mVSubZero_100turn2mmFR_VC93p5_FieldCompNoSwitch_LS'
...
for i_pixel in range(0, length_file, 1):
    for i2 in range(start_file_freq, end_file_freq+1, increment_file_freq):
        if freq[i2] in avoid_freq:
            ...
            #print(i2)
            #print(freq[i2])
            fitted_parameters_X, pcov = curve_fit(dispersivelorentzian_or_lorentzian, f1[0], f1[1], p0 = [1E-4, freq[i2]-0.1, 0.1, 0, 1E-3], bounds=([1E-6, freq[i2]-0.2, 0.05, -2*np.pi, -0.01], [0.01, freq[i2]+0.1, 0.15, 2*np.pi, 0.01]))
            ...
            #f1_all_fitted[i2][i_pixel] = 4 * fitted_parameters_Y[0] / fitted_parameters_Y[2]
            #f2_all_fitted[i2][i_pixel] = fitted_parameters_Y[3]
            #f3_all_fitted[i2][i_pixel] = fitted_parameters_Y[2]
            #f4_all_fitted[i2][i_pixel] = fitted_parameters_Y[1]
            ...
            #ax_fit.plot(f1[0], np.sqrt(f1[1]**2+f1[2]**2), label='Exp R', color='black')
...
for i2 in range(start_file_freq, end_file_freq+1, increment_file_freq):
    if freq[i2] in avoid_freq:
        ...
        ax_fits_1.plot(x, f1_all_fitted[i2][:] * 1 / (f1_all_fitted[i2][0]+f1_all_fitted[i2][len(x)-1])/2, label='%skHz' % freq[i2])
...
np.savetxt('%s/testFittedSpectra_Ampl_Parts%s-%s.dat' % (path_directory, start_file, end_file), f1_all_fitted[:][:])
np.savetxt('%s/testFittedSpectra_Phase_Parts%s-%s.dat' % (path_directory, start_file, end_file), f2_all_fitted[:][:] * 180/np.pi)
np.savetxt('%s/testFittedSpectra_Linewidth_Parts%s-%s.dat' % (path_directory, start_file, end_file), f3_all_fitted[:][:])
np.savetxt('%s/testFittedSpectra_Larmor_Parts%s-%s.dat' % (path_directory, start_file, end_file), f4_all_fitted[:][:])
