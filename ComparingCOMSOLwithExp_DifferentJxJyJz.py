...
comsol_sigma_of_interest = 0.001#10E6# 0.25E6 + 2 * 0.2E6
    ...
    for i3 in range(len(param_sweep_sheet_conductivity)):
        for i4 in range(len(param_sweep_sheet_height)):
            for i in range(len(param_sweep_freq)):
                ...
                denominator = bx_abs**2 + by_abs**2 + 2 * bx_abs * np.sin(bx_theta) * by_abs * np.cos(bx_theta) - 2 * bx_abs * np.cos(bx_theta) * by_abs * np.sin(by_theta)
                X = (np.abs(bx) * np.cos(bx_theta) - np.abs(by) * np.sin(by_theta))/denominator
                Y = (- np.abs(bx) * np.sin(bx_theta) - np.abs(by) * np.cos(by_theta))/denominator
                print('bx_abs**2', bx_abs[0]**2)
                print('by_abs**2', by_abs[0]**2)
                print('2 * bx_abs * np.sin(bx_theta) * by_abs * np.cos(bx_theta)', 2 * bx_abs[0] * np.sin(bx_theta[0]) * by_abs[0] * np.cos(bx_theta[0]))
                ...
                if param_sweep_freq[i] in avoid_freq:
                    ...
                    if param_sweep_sheet_conductivity[i3]==comsol_sigma_of_interest and param_sweep_sheet_permeability[i2]==comsol_mur_of_interest:                
                        ...
                        ax5.plot(plate_length_conc, np.sqrt(np.real(bx_conc)**2+np.imag(bx_conc)**2)*1/(np.sqrt(np.real(by_conc[0])**2+np.imag(by_conc[0])**2)), label='%skHz, $\mu_{r}=%s$, $\sigma=%s$MS/m' % (param_sweep_freq[i], param_sweep_sheet_permeability[i2], param_sweep_sheet_conductivity[i3]/10**6))
                        ax6.plot(plate_length_conc, np.unwrap(180 / np.pi * np.arctan2(np.imag(bx_conc), np.real(bx_conc)), period=360), label='%skHz' % (param_sweep_freq[i]))
                        ax7.plot(plate_length_conc, np.sqrt(np.real(by_conc)**2+np.imag(by_conc)**2)*1/(np.sqrt(np.real(by_conc[0])**2+np.imag(by_conc[0])**2)), label='%skHz, $\mu_{r}=%s$, $\sigma=%s$MS/m' % (param_sweep_freq[i], param_sweep_sheet_permeability[i2], param_sweep_sheet_conductivity[i3]/10**6))
                        ax8.plot(plate_length_conc, 180/np.pi*np.arctan2(np.imag(by_conc), np.real(by_conc)), label='%skHz' % (param_sweep_freq[i]))
                ...
                if param_sweep_freq[i] in avoid_freq:
                    ...
                    if param_sweep_sheet_conductivity[i3]==comsol_sigma_of_interest and param_sweep_sheet_permeability[i2]==comsol_mur_of_interest:                
                        ...
                        ax20.plot(plate_length_conc, R * 1 / (R[0]), label='COM, %skHz' % (param_sweep_freq[i]))
                        ax21.plot(plate_length_conc, 180/np.pi * Phase, label='COM, %skHz' % (param_sweep_freq[i]))
...
ax5.axvline(x=-75, color='black')
ax5.axvline(x=75, color='black')
ax6.axvline(x=-75, color='black')
ax6.axvline(x=75, color='black')
ax7.axvline(x=-75, color='black')
ax7.axvline(x=75, color='black')
ax8.axvline(x=-75, color='black')
ax8.axvline(x=75, color='black')
ax20.axvline(x=-75, color='black')
ax20.axvline(x=75, color='black')
ax21.axvline(x=-75, color='black')
ax21.axvline(x=75, color='black')
ax1.grid()
ax2.grid()
ax3.grid()
...
ax18.set_ylabel('$X$')
ax19.set_ylabel('$Y$')
ax20.set_ylabel('$R$ (norm)')
ax21.set_ylabel('Phase ($\degree$)')
