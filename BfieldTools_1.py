...
plate_separation = 2
standoff = np.array([0, plate_separation, 0])
...
# This specifies your point of interest where you want the uniformity or maximum field or whatever
...
#print('target_field[0, 1]', target_field[0, 1])
#print('target_field[1, 0]', target_field[1, 0])
#print('target_field[2, 1]', target_field[2, 0])
...
coil.s2 = optimize_lsq(
    coil, [target_spec, stray_spec], objective="minimum_inductive_energy", reg=1e6
)
...
# Discretizing optimized stream function
loops2 = coil.s2.discretize(N_contours=10)

# Plotting discretized coil windings
loops2.plot_loops()

B_target = loops2.magnetic_field(target_points)
# Plotting resultant magnetic field
mlab.quiver3d(*target_points.T, *B_target.T)
...
# Very simply defining x and y coordinates (24x24 grid)
x = y = np.linspace(-8, 8, 250)
...
# Linewidth of streamplots based on strength of B=sqrt(Bx^2+By^2)
lw = np.sqrt(B[0] ** 2 + B[1] ** 2)
lw = 2 * lw / np.max(lw)

# Red and blue heatmap on final figure cross-section
plot_cross_section(X, Y, U, log=False, contours=False)

# If you don't have following then the streamplot lines are not positioned correctly in the figure
seed_points = points[:, :2] * 0.3

# Black streamplot lines
plt.streamplot(
    x,
    y,
    B[0],
...
plt.plot([-5, 5], [-plate_separation, -plate_separation], "k", linewidth=3, alpha=1)
plt.plot([-5, 5], [plate_separation, plate_separation], "k", linewidth=3, alpha=1)
