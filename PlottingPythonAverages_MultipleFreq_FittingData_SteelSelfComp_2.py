...
# File path
path_directory = 'P:/Coldatom/RafalGartman/231214'
...
path_file = 'SingPh_B0V_24mmStWritingAwayComp_2Vsubzero_FieldCpSwitchRF1_LSDoor'
...
length_file = 129#len(np.loadtxt('%s/Part%s_%s/%s%s.dat' % (path_directory, start_file, path_file, path_file_name, 0)))
...
max_R = np.zeros((len(freq), length_file))
...
for i_pixel in range(0, length_file, 1):
    for i2 in range(start_file_freq, end_file_freq+1, increment_file_freq):
        if freq[i2] in avoid_freq:
            ...
            fitted_parameters_X, pcov = curve_fit(dispersivelorentzian_or_lorentzian, f1[0], f1[2], p0 = [1E-5, freq[i2]-0.005, 0.1, np.pi, 0], bounds=([1E-6, freq[i2]-1, 0.05, 0, -0.01], [0.005, freq[i2]+1, 1, 2*np.pi, 0.01]))
            ...
            max_R[i2][i_pixel] = max(np.sqrt(f1[1]**2+f1[2]**2))
...
calibrated_values_131223 = [1.52E-03, 1.30E-03, 1.12E-03, 3.65E-04]
for i2 in range(start_file_freq, end_file_freq+1, increment_file_freq):
    if freq[i2] in avoid_freq:
        trial = 0
    else:   
        #ax_fits_1.plot(x, f1_all_fitted[i2][:], label='%skHz' % freq[i2])
        #ax_fits_1.plot(x, max_R[i2][:]/calibrated_values_131223[i2], label='%skHz' % freq[i2])
        ax_fits_1.plot(x, max_R[i2][:]/calibrated_values_131223[i2]*f3_all_fitted[i2][:], label='%skHz' % freq[i2])
