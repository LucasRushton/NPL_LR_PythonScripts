...
path1 = 'P:/Coldatom/LucasRushton/COMSOL/FieldFocussing/240327_11Coils_Overnight_VaryingCurrent2_Fine'
textfile_name = '240327_11Coils_Overnight_VaryingCurrent2_Fine_Raw'

param_sweep_defect_position = np.arange(-0.050, 0.010001, 0.0005)
#print(param_sweep_defect_position)
#print(param_sweep_defect_position, len(param_sweep_defect_position))

param_sweep_liftoff = np.array([-3, -4, -5, -6]) #np.arange(0.0005, 0.004001, 0.0005)
comsol_step = 1
#param_sweep_sheet_height = [1]

data = np.transpose(np.loadtxt('%s/%s.csv' % (path1, textfile_name), delimiter=',', dtype=np.complex_))
...
for i in range(len(param_sweep_liftoff)):
    bx = data[9][counter*len(param_sweep_defect_position):len(param_sweep_defect_position)+counter*len(param_sweep_defect_position)]
    by = data[10][counter*len(param_sweep_defect_position):len(param_sweep_defect_position)+counter*len(param_sweep_defect_position)]
    ...
    b_abs = np.sqrt(bx_abs**2+by_abs**2)
    
    #b_frac = bx_abs / bz_abs
    
    ax1.plot(param_sweep_defect_position*100, np.real(bx), label='Current = %s A' % round(param_sweep_liftoff[i]))
    ax1.set_ylabel('Re[$B_{x}$] (T)')
    ax1.set_xlabel('Position of plate edge (cm)')
    
    ax_all.plot(param_sweep_defect_position*100, np.real(bx), label='Current = %s A' % round(param_sweep_liftoff[i]))
    ax_all.set_ylabel('Re[$B_{x}$] (T)')
    ax_all.set_xlabel('Position of plate edge (cm)')
    ...
    ax2.set_xlabel('Position of plate edge (cm)')
    ax2.grid()
    ax2.legend()
    #ax2.set_yscale('log')
...
ax1.scatter(np.array(max_position[0:2])*100, max_bx[0:2], marker='x', label='Predicted plate edge', color='blue')
ax1.scatter(np.array([-0.00331, -0.002019])*100, np.array([2.7880E-7, -2.48E-8]), marker='x', color='blue')
ax_all.scatter(np.array(max_position[0:2])*100, max_bx[0:2], marker='x', label='Predicted plate edge', color='blue')
ax_all.scatter(np.array([-0.00331, -0.002019])*100, np.array([2.7880E-7, -2.48E-8]), marker='x', color='blue')


#ax1.scatter(fwhm_pos_1, fwhm_bx_1, marker='x', color='black')
#ax1.scatter(fwhm_pos_2, fwhm_bx_2, marker='x', color='black')
#ax1.plot()
ax1.set_title('Increasing current to coils adjacent to 10 A central coil; Liftoff = 8 mm')
ax1.axvline(x=0, label='Central coil position', linestyle='dashed')
ax1.legend()
ax1.grid()

ax_all.set_title('Increasing current to coils adjacent to 10 A central coil; Liftoff = 8 mm')
ax_all.axvline(x=0, label='Central coil position', linestyle='dashed')
ax_all.grid()
#plt.show()
...
# Don't really have to adjust following till end of programme, unless you're a keen bean!!!
path1 = 'P:/Coldatom/LucasRushton/COMSOL/FieldFocussing/240327_1Coil_Overnight_VaryingLiftoff_Fine'
textfile_name = '240327_1Coil_Overnight_VaryingLiftoff_Fine_Raw'

param_sweep_defect_position = np.arange(-0.050, 0.020001, 0.0005)
#print(param_sweep_defect_position)
#print(param_sweep_defect_position, len(param_sweep_defect_position))

param_sweep_liftoff = np.array([0.001, 0.002, 0.003, 0.004, 0.005, 0.006, 0.007, 0.008, 0.009, 0.010]) #np.arange(0.0005, 0.004001, 0.0005)
comsol_step = 1
#param_sweep_sheet_height = [1]
...
for i in range(len(param_sweep_liftoff)):
    ...
    ax1.plot(param_sweep_defect_position*100, np.real(bx), label='Liftoff = %s mm' % round(param_sweep_liftoff[i]*1000))
    ax1.set_ylabel('Re[$B_{x}$] (T)')
    ax1.set_xlabel('Position of plate edge (cm)')
    
    if round(param_sweep_liftoff[i]*1000)==8:
        ax_all.plot(param_sweep_defect_position*100, np.real(bx), label='Single coil MIT')
        print('hi')
    ...
    #print(fwhm_2)
    if round(param_sweep_liftoff[i]*1000)==8:
        ax_all.scatter(param_sweep_defect_position[np.argmax(bx_abs)]*100, max(bx_abs), marker='x', color='blue')
...
ax1.scatter(np.array(max_position)*100, max_bx, marker='x', label='Predicted plate edge', color='blue')
...
#ax1.scatter(fwhm_pos_2, fwhm_bx_2, marker='x', color='black')
#ax1.plot()
ax1.set_title('Single coil MIT with varying liftoffs')
ax1.axvline(x=0, label='Central coil position', linestyle='dashed')
ax1.legend()
ax1.grid()
plt.show()
...
# Mapping out the RF fields

comsol_m3A = np.transpose(np.loadtxt(r'P:\Coldatom\LucasRushton\COMSOL\FieldFocussing\240327_11Coils_MappingRFField\COMSOL_ReBx_-3A.txt'))
comsol_m4A = np.transpose(np.loadtxt(r'P:\Coldatom\LucasRushton\COMSOL\FieldFocussing\240327_11Coils_MappingRFField\COMSOL_ReBx_-4A.txt'))
comsol_m5A = np.transpose(np.loadtxt(r'P:\Coldatom\LucasRushton\COMSOL\FieldFocussing\240327_11Coils_MappingRFField\COMSOL_ReBx_-5A.txt'))
comsol_m6A = np.transpose(np.loadtxt(r'P:\Coldatom\LucasRushton\COMSOL\FieldFocussing\240327_11Coils_MappingRFField\COMSOL_ReBx_-6A.txt'))

fig = plt.figure()
ax = fig.add_subplot(111)

ax.plot(comsol_m3A[0]*100 - 100*comsol_m3A[0][np.argmax(comsol_m3A[1])], comsol_m3A[1]/max(comsol_m3A[1]), label='Current = -3 A')
ax.plot(comsol_m4A[0]*100 - 100*comsol_m4A[0][np.argmax(comsol_m4A[1])], comsol_m4A[1]/max(comsol_m4A[1]), label='Current = -4 A')
ax.plot(comsol_m5A[0]*100 - 100*comsol_m5A[0][np.argmax(comsol_m5A[1])], comsol_m5A[1]/max(comsol_m5A[1]), label='Current = -5 A')
ax.plot(comsol_m5A[0]*100 - 100*comsol_m6A[0][np.argmax(comsol_m6A[1])], comsol_m6A[1]/max(comsol_m6A[1]), label='Current = -6 A')

ax.set_ylabel('Re[$B_{x}$] (norm)')
ax.set_xlabel('Position relative to central coil (cm)')
ax.grid()
ax.legend()
ax.set_title('RF field distribution without plate at 8 mm liftoff')
plt.show()
#print(bx)
'''
fig2 = plt.figure()
